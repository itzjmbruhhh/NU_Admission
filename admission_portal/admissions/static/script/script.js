...
function formatMobileNumber(input) {
  ...
  // Format as: XXXX XXX XXXX (0920 299 2766)
  let formatted = "";
  if (digits.length > 0) {
...
// Setup mobile number formatting with better event handling
...
// Setup Navigation Buttons (CSP-compliant event listeners)
...
function updateSectionVisibility(direction = 1) {
  ...
  sections.forEach((section, index) => {
    ...
    if (index < currentSection) {
      ...
      if (direction > 0) {
        section.classList.add("entering-forward");
      } else if (direction < 0) {
        section.classList.add("entering-backward");
      }
    }
  });
}
...
function updateNavigation() {
  ...
  if (sectionInfo) {
    sectionInfo.textContent = `Step ${currentSection + 1} of ${totalSections}`;
  }
}
...
// Save current section fields into localStorage as JSON
...
// Load draft from localStorage and populate fields
...
// Clear saved draft only when user refreshed the page
...
function setupModalHandlers() {
  ...
  const successMeta = document.querySelector('meta[name="submission_success"]');
...
// Setup Location Data (PSGC API logic)
...
// Setup Address Sync ("same as present" logic)
